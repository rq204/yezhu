...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
        ...
        */
        ...
        bool IsSqlite() { return _connectionString.IndexOf("sqlite",  StringComparison.OrdinalIgnoreCase)>-1; }
        ...
        public IEnumerable<T> QuerySingleColumn<T>(string sql, params object[] args) /*where T : new()*/
        {
            if (EnableAutoSelect)
                sql = AddSelectClause<T>(sql);
                    ...
                    using (r)
                    {
                        while (true)
                        {
                            ...
                            try
                            {
                                ...
                                poco = (T)Convert.ChangeType(val, typeof(T));
                            }
                            catch (Exception x)
                            {
                                OnException(x);
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        foreach (var i in data)
                        {
                            ...
                            AddParam(cmd, i.Value, _paramPrefix);
                        }
			...
			try
			{
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
                        ...
                        var sqlGetNewId = "SELECT @@IDENTITY AS NewID";
                        
                        if (IsSqlite())
                        {
                            sqlGetNewId = "select last_insert_rowid() as  'identity'";
                        }
					...
					lock (m_Converters)
					{
						...
						for (int i = 0; i < r.FieldCount; i++)
						{
							...
							if (converter == null && !dstType.IsAssignableFrom(srcType))
							{
                                ...
                                {
                                    if (dstType.IsEnum)//add rq204
                                    {
                                        if (src is string)
                                            return Enum.Parse(dstType, src as string);
                                        else
